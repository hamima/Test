                ...
                [
                    {
                        ...
                        columns:  [
                            ...
                            {
                                xtype: 'gridcolumn',
                                text: 'گروه اقلام مصرفی',
                                emptyCellText: '---',
                                dataIndex:'consumingGoodsGroup',
                                renderer: function(value){//1.
                                    var consumingGoodsGroupName;
                                    var consumingGoodsGroup = value;
                                    if(consumingGoodsGroup){
                                        if(consumingGoodsGroup.description){
                                        	consumingGoodsGroupName = consumingGoodsGroup.description;
                                        }else{
                                            var combo = Ext.getCmp('consumingGoodsGroupListCombo');
                                            var rec = combo.findRecord(combo.valueField , value);
                                            if(rec){
                                            	consumingGoodsGroupName = rec.data.description;
                                            }else{
                                            	consumingGoodsGroupName = '---';
                                            }
                                        }
                                    } else{
                                    	consumingGoodsGroupName = '---';
                                    }
                                    return consumingGoodsGroupName;
                                },
                                editor: {
                                    xtype: 'combobox',
                                    id:'consumingGoodsGroupListCombo',
